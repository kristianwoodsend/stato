#! /usr/bin/env python

import argparse
from util import *
from match import match_player_ids, avg_team
from optimise import optimise, NFL_CONFIG
from download import projection_sources, get_data, get_random
from data import load_projections, list_sources
import csv


def archive(file_name):
    with open(file_name, 'wb') as csvfile:
        writer = csv.writer(csvfile, delimiter=',', quotechar='"')

        writer.writerow(["Source", "ID", "Name", "Position", "Team", "Salary", "fp"])

        fd_players = load_obj("fd_players")
        matched_players = [(title, match_player_ids(fd_players, load_obj(source)))
                           for title, source, _ in projection_sources]

        for title, players, in matched_players:
            for p in players:
                writer.writerow([title, p.id, p.name, p.position, p.team_code, p.salary, p.fp])


def optimize_team(args):
    fd_players = load_obj("fd_players")
    if args.xp:
        fd_players = filter(lambda p: p.id not in args.xp, fd_players)

    exclude_sources = args.xs or []

    matched_players = [(title, match_player_ids(fd_players, load_obj(source)))
                       for title, source, _ in projection_sources
                       if source not in exclude_sources]

    for title, src_players, in matched_players:
        score, team = optimise(src_players, force_players=args.fp, stack_teams=args.stack)
        print_team(title, team, score)

    avg = avg_team(fd_players, [team for _, team in matched_players])
    score, team = optimise(avg, force_players=args.fp, stack_teams=args.stack)
    print_team('Averaged', team, score)


def with_rnd(args):
    fd_players = load_obj("fd_players")
    if args.xp:
        fd_players = filter(lambda p: p.id not in args.xp, fd_players)

    exclude_sources = args.xs or []

    projection_sources.append(("Random", "rnd_data", get_random))

    for x in range(int(args.rnd)):
        get_random("rnd_data")
        matched_players = [(title, match_player_ids(fd_players, load_obj(source)))
                           for title, source, _ in projection_sources
                           if source not in exclude_sources]

        avg = avg_team(fd_players, [team for _, team in matched_players])
        score, team = optimise(avg, force_players=args.fp, stack_teams=args.stack)
        print_team('Averaged', team, score)


def list_projection_sources():
    print '\nAvailable projection sources;'
    for title in list_sources():
        print '{}'.format(title)


def list_player(args):
    fd_players = load_obj("fd_players")
    players = match_player_ids(fd_players, load_obj(args.p))
    print '\nPlayer projections from {}'.format(args.p)
    print_player_list(players)


def list_pos(args):
    fd_players = load_obj("fd_players")
    matched_players = [(title, match_player_ids(fd_players, load_obj(source)))
                       for title, source, _ in projection_sources]

    for title, team, in matched_players:
        print '\n{}'.format(title)
        print_all_in_pos(team, args.pos)

    avg = avg_team(fd_players, [team for _, team in matched_players])
    print '\nAveraged'
    print_all_in_pos(avg, args.pos)


def stack_team(s):
    try:
        team_code, number = s.split(':')
        return team_code, int(number)
    except:
        raise argparse.ArgumentTypeError("Stacked team must be teamcode:min_players")


def main():
    parser = argparse.ArgumentParser(description="NFL Team Optimiser")
    parser.add_argument('-src', metavar='file_name', help='CSV file with source projections')
    parser.add_argument('-res', metavar='file_name', help='CSV file with results')
    parser.add_argument('-arc', metavar='file_name', help='Archive current player projections')
    parser.add_argument('-refresh', help='Download projections again')
    parser.add_argument('-ls', default=False, action='store_true',
                        help='List available sources')
    parser.add_argument('-p', metavar='src', help='List players for a source')
    parser.add_argument('-pos', metavar='pos', help='List players in a position')
    parser.add_argument('-xp', metavar='pid', nargs='+', help='Exclude players from optimisations')
    parser.add_argument('-xs', metavar='src', nargs='+', help='Exclude projection source from optimisations')
    parser.add_argument('-fp', metavar='pid', nargs='+', help='Force specific player for optimisations')
    parser.add_argument('-stack', metavar='team_code:min', nargs='+', type=stack_team, help='Stack a specific team')
    parser.add_argument('-rnd', metavar='iterations', help='Include a random team')

    args = parser.parse_args()

    get_data()

    # if not args.src:
    #     raise Exception("no src projections specified")
    #
    # load_projections(args.src)

    if args.ls:
        list_projection_sources()
    elif args.p:
        list_player(args)
    elif args.pos:
        list_pos(args)
    elif args.arc:
        archive(args.arc)
    elif args.rnd:
        with_rnd(args)
    else:
        optimize_team(args)

if __name__ == "__main__":
    main()
